...
static void DoDistributorInit(XScuGic* InstancePtr)
{
    ...
    for (Int_Id = 32U; Int_Id < XSCUGIC_MAX_NUM_INTR_INPUTS; Int_Id = Int_Id + 16U)
    {
         ...
         */
        XScuGic_DistWriteReg(InstancePtr, XSCUGIC_INT_CFG_OFFSET_CALC(Int_Id), 0U);
    }
...
#define DEFAULT_PRIORITY 0xa0a0a0a0U
    for (Int_Id = 0U; Int_Id < XSCUGIC_MAX_NUM_INTR_INPUTS; Int_Id = Int_Id + 4U)
    {
         ...
         */
        XScuGic_DistWriteReg(InstancePtr, XSCUGIC_PRIORITY_OFFSET_CALC(Int_Id), DEFAULT_PRIORITY);
    }

#if defined(GICv3)
    for (Int_Id = 0U; Int_Id < XSCUGIC_MAX_NUM_INTR_INPUTS; Int_Id = Int_Id + 32U)
    {
        XScuGic_DistWriteReg(InstancePtr, XSCUGIC_SECURITY_TARGET_OFFSET_CALC(Int_Id),
    ...
    for (Int_Id = 0U; Int_Id < XSCUGIC_MAX_NUM_INTR_INPUTS; Int_Id = Int_Id + 32U)
    {
        ...
        XScuGic_DistWriteReg(
            InstancePtr, XSCUGIC_EN_DIS_OFFSET_CALC(XSCUGIC_DISABLE_OFFSET, Int_Id), 0xFFFFFFFFU);
    }
#if defined(GICv3)
    Temp = XScuGic_DistReadReg(InstancePtr, XSCUGIC_DIST_EN_OFFSET);
    Temp |= XSCUGIC_EN_INT_MASK;
...
s32 XScuGic_CfgInitialize(XScuGic* InstancePtr, XScuGic_Config* ConfigPtr, u32 EffectiveAddr)
{
    ...
    {
        Xil_AssertNonvoid(
            (Xil_In32(XPS_EFUSE_BASEADDR + EFUSE_STATUS_OFFSET) & EFUSE_STATUS_CPU_MASK) == 0);
    }
#endif
...
/* FOR freertos SMP port */
s32 XScuGic_CpuIfInitialize(XScuGic* InstancePtr)
{
    CPUInitialize(InstancePtr);
    return XST_SUCCESS;
}
 ...
 *
 ****************************************************************************/
s32 XScuGic_Connect(XScuGic* InstancePtr, u32 Int_Id, Xil_InterruptHandler Handler,
                    void* CallBackRef)
{
    /*
...
void XScuGic_SetPriorityTriggerType(XScuGic* InstancePtr, u32 Int_Id, u8 Priority, u8 Trigger)
{
    ...
    if (Int_Id < XSCUGIC_SPI_INT_ID_START)
    {
        XScuGic_ReDistSGIPPIWriteReg(InstancePtr, XSCUGIC_RDIST_INT_PRIORITY_OFFSET_CALC(Int_Id),
                                     Priority);
        Temp =
            XScuGic_ReDistSGIPPIReadReg(InstancePtr, XSCUGIC_RDIST_INT_CONFIG_OFFSET_CALC(Int_Id));
        Index = XScuGic_Get_Rdist_Int_Trigger_Index(Int_Id);
        Temp |= (Trigger << Index);
        XScuGic_ReDistSGIPPIWriteReg(InstancePtr, XSCUGIC_RDIST_INT_CONFIG_OFFSET_CALC(Int_Id),
                                     Temp);
        return;
    }
#endif
     ...
     */
    RegValue = XScuGic_DistReadReg(InstancePtr, XSCUGIC_PRIORITY_OFFSET_CALC(Int_Id));
     ...
     */
    XScuGic_DistWriteReg(InstancePtr, XSCUGIC_PRIORITY_OFFSET_CALC(Int_Id), RegValue);

    /*
     * Determine the register to write to using the Int_Id.
     */
    RegValue = XScuGic_DistReadReg(InstancePtr, XSCUGIC_INT_CFG_OFFSET_CALC(Int_Id));
     ...
     * Write the value back to the register.
     */
    XScuGic_DistWriteReg(InstancePtr, XSCUGIC_INT_CFG_OFFSET_CALC(Int_Id), RegValue);
}

void XScuGic_SetPPI_SGI_Priority(XScuGic* InstancePtr, u32 Int_Id, u32 Priority)
{
    u32 RegValue;
#if defined(GICv3)
    u32 Temp;
...
void XScuGic_InterruptMaptoCpu(XScuGic* InstancePtr, u8 Cpu_Id, u32 Int_Id)
{
    ...
    {
        Temp = Int_Id - 32;
        RegValue = XScuGic_DistReadReg(InstancePtr, XSCUGIC_IROUTER_OFFSET_CALC(Temp));
        RegValue |= Cpu_Id;
        XScuGic_DistWriteReg(InstancePtr, XSCUGIC_IROUTER_OFFSET_CALC(Temp), (Cpu_Id - 1));
    }
#else
    u32 Offset;
    Xil_AssertVoid(InstancePtr != NULL);
...
void XScuGic_UnmapAllInterruptsFromCpu(XScuGic* InstancePtr, u8 Cpu_Id)
{
    ...
    for (Int_Id = 32U; Int_Id < XSCUGIC_MAX_NUM_INTR_INPUTS; Int_Id = Int_Id + 4U)
    {
        ...
        XScuGic_DistWriteReg(InstancePtr, XSCUGIC_SPI_TARGET_OFFSET_CALC(Int_Id), Target_Cpu);
    }
}
/****************************************************************************/
/**
