...
void* mappend(void** p, size_t size, size_t* count, size_t* capacity)
{
    uint8_t* p_ = *p;
    size_t count_ = *count;
...
{
    const char* name;
    void (*run)(const lfs_emubd_powercycles_t* cycles, size_t cycle_count,
                const struct test_suite* suite, const struct test_case* case_);
...
#define TEST_SUITE_COUNT ((size_t)(&__stop__test_suites - &__start__test_suites))
...
#define TEST_DEF(k, v) [k##_i] = {test_define_##k, NULL},
...
const char* test_define_name(size_t define)
{
    ...
    {
        if (define < test_define_names[i].count && test_define_names[i].names &&
...
bool test_define_ispermutation(size_t define)
{
    ...
    {
        if (define < test_define_maps[i].count && test_define_maps[i].defines[define].cb)
        {
            return true;
        }
    }
...
intmax_t test_define(size_t define)
{
    ...
    for (size_t i = 0; i < TEST_DEFINE_MAP_COUNT; i++)
    {
        ...
        {
            intmax_t v =
                test_define_maps[i].defines[define].cb(test_define_maps[i].defines[define].data);
...
void test_define_flush(void)
{
    ...
    memset(test_define_cache_mask, 0,
           sizeof(unsigned) *
               ((test_define_cache_count + (8 * sizeof(unsigned)) - 1) / (8 * sizeof(unsigned))));
}
...
{
    test_define_maps[TEST_DEFINE_MAP_GEOMETRY] =
        (test_define_map_t){geometry->defines, TEST_GEOMETRY_DEFINE_COUNT};
}
...
{
    test_define_names[TEST_DEFINE_NAMES_SUITE] =
        (test_define_names_t){suite->define_names, suite->define_count};

    // make sure our cache is large enough
    if (lfs_max(suite->define_count, TEST_IMPLICIT_DEFINE_COUNT) > test_define_cache_count)
    {
        // align to power of two to avoid any superlinear growth
        size_t ncount = 1 << lfs_npw2(lfs_max(suite->define_count, TEST_IMPLICIT_DEFINE_COUNT));
        test_define_cache = realloc(test_define_cache, ncount * sizeof(intmax_t));
        test_define_cache_mask = realloc(
            test_define_cache_mask,
            sizeof(unsigned) * ((ncount + (8 * sizeof(unsigned)) - 1) / (8 * sizeof(unsigned))));
    ...
    if (test_override_count > 0)
    {
        ...
        {
            for (size_t d = 0; d < lfs_max(suite->define_count, TEST_IMPLICIT_DEFINE_COUNT); d++)
            {
                // define name match?
        ...
        if (count * permutations > test_override_define_capacity)
        {
            ...
            test_override_defines =
                realloc(test_override_defines, sizeof(test_define_t) * ncapacity);
            test_override_define_capacity = ncapacity;
        }

        // zero unoverridden defines
        memset(test_override_defines, 0, sizeof(test_define_t) * count * permutations);
                ...
                if (name && strcmp(name, test_overrides[i].name) == 0)
                {
                    ...
                    for (size_t j = 0; j < permutations; j++)
                    {
                        test_override_defines[j * count + d] = TEST_LIT(
                            test_overrides[i].defines[(j / p) % test_overrides[i].permutations]);
                    }
...
void test_define_perm(const struct test_suite* suite, const struct test_case* case_, size_t perm)
{
    if (case_->defines)
    {
        test_define_maps[TEST_DEFINE_MAP_PERMUTATION] =
            (test_define_map_t){case_->defines + perm * suite->define_count, suite->define_count};
    }
    else
    {
        test_define_maps[TEST_DEFINE_MAP_PERMUTATION] = (test_define_map_t){NULL, 0};
    }
}
...
void test_define_override(size_t perm)
{
    test_define_maps[TEST_DEFINE_MAP_OVERRIDE] = (test_define_map_t){
        test_override_defines + perm * test_override_define_count, test_override_define_count};
}

void test_define_explicit(const test_define_t* defines, size_t define_count)
{
    test_define_maps[TEST_DEFINE_MAP_EXPLICIT] = (test_define_map_t){defines, define_count};
}
...
#endif
void* test_trace_backtrace_buffer[TEST_TRACE_BACKTRACE_BUFFER_SIZE / sizeof(void*)];
...
void test_trace(const char* fmt, ...)
{
    if (test_trace_path)
    {
        ...
        if (test_trace_freq)
        {
            ...
            uint64_t now = (uint64_t)t.tv_sec * 1000 * 1000 * 1000 + (uint64_t)t.tv_nsec;
        ...
        if (!test_trace_file)
        {
            ...
            {
                fd = open(test_trace_path, O_WRONLY | O_CREAT | O_APPEND | O_NONBLOCK, 0666);
                if (fd < 0)
                {
                    return;
                }
            ...
            FILE* f = fdopen(fd, "a");
            assert(f);
            int err = setvbuf(f, NULL, _IOFBF, TEST_TRACE_BACKTRACE_BUFFER_SIZE);
            assert(!err);
            test_trace_file = f;
        }
        ...
        {
            // print backtrace
            size_t count = backtrace(test_trace_backtrace_buffer, TEST_TRACE_BACKTRACE_BUFFER_SIZE);
...
static void perm_printid(const struct test_suite* suite, const struct test_case* case_,
                         const lfs_emubd_powercycles_t* cycles, size_t cycle_count)
{
    (void)suite;
...
bool test_seen_insert(test_seen_t* seen, const struct test_suite* suite,
                      const struct test_case* case_)
{
    ...
    for (size_t d = 0; d < lfs_max(suite->define_count, TEST_IMPLICIT_DEFINE_COUNT); d++)
    {
        ...
        {
            was_seen = false;
            branch = mappend((void**)&seen->branches, sizeof(struct test_seen_branch),
                             &seen->branch_count, &seen->branch_capacity);
...
static void run_powerloss_none(const lfs_emubd_powercycles_t* cycles, size_t cycle_count,
                               const struct test_suite* suite, const struct test_case* case_);
static void run_powerloss_cycles(const lfs_emubd_powercycles_t* cycles, size_t cycle_count,
                                 const struct test_suite* suite, const struct test_case* case_);

// iterate through permutations in a test case
static void case_forperm(const struct test_suite* suite, const struct test_case* case_,
                         const test_define_t* defines, size_t define_count,
                         const lfs_emubd_powercycles_t* cycles, size_t cycle_count,
                         void (*cb)(void* data, const struct test_suite* suite,
                                    const struct test_case* case_,
    ...
    if (defines)
    {
        ...
        for (size_t v = 0; v < test_override_define_permutations; v++)
        {
            ...
            if (cycles)
            {
                cb(data, suite, case_,
                   &(test_powerloss_t){
                       .run = run_powerloss_cycles, .cycles = cycles, .cycle_count = cycle_count});
            }
            else
            {
    ...
    for (size_t k = 0; k < case_->permutations; k++)
    {
        ...
        for (size_t v = 0; v < test_override_define_permutations; v++)
        {
            ...
            for (size_t g = 0; g < test_geometry_count; g++)
            {
                ...
                {
                    cb(data, suite, case_,
                       &(test_powerloss_t){.run = run_powerloss_cycles,
                                           .cycles = cycles,
...
void perm_count(void* data, const struct test_suite* suite, const struct test_case* case_,
                const test_powerloss_t* powerloss)
{
...
static void summary(void)
{
    ...
    for (size_t t = 0; t < test_id_count; t++)
    {
        for (size_t i = 0; i < TEST_SUITE_COUNT; i++)
        {
            ...
            for (size_t j = 0; j < test_suites[i].case_count; j++)
            {
                ...
                if (test_ids[t].name &&
                    !(strcmp(test_ids[t].name, test_suites[i].name) == 0 ||
                      strcmp(test_ids[t].name, test_suites[i].cases[j].name) == 0))
                {
                    continue;
                }

                cases += 1;
                case_forperm(&test_suites[i], &test_suites[i].cases[j], test_ids[t].defines,
                             test_ids[t].define_count, test_ids[t].cycles, test_ids[t].cycle_count,
                             perm_count, &perms);
            }
    ...
    char flag_buf[64];
    sprintf(flag_buf, "%s%s", (flags & TEST_REENTRANT) ? "r" : "", (!flags) ? "-" : "");
    printf("%-23s  %7s %7zu %7zu %11s\n", "TOTAL", flag_buf, suites, cases, perm_buf);
}
...
static void list_suites(void)
{
    ...
    for (size_t t = 0; t < test_id_count; t++)
    {
        for (size_t i = 0; i < TEST_SUITE_COUNT; i++)
        {
            ...
            sprintf(perm_buf, "%zu/%zu", perms.filtered, perms.total);
            char flag_buf[64];
            sprintf(flag_buf, "%s%s", (test_suites[i].flags & TEST_REENTRANT) ? "r" : "",
                    (!test_suites[i].flags) ? "-" : "");
            printf("%-*s  %7s %7zu %11s\n", name_width, test_suites[i].name, flag_buf, cases,
                   perm_buf);
        }
    }
}
        ...
        for (size_t i = 0; i < TEST_SUITE_COUNT; i++)
        {
            ...
            for (size_t j = 0; j < test_suites[i].case_count; j++)
            {
                ...
                sprintf(flag_buf, "%s%s",
                        (test_suites[i].cases[j].flags & TEST_REENTRANT) ? "r" : "",
                        (!test_suites[i].cases[j].flags) ? "-" : "");
                printf("%-*s  %7s %11s\n", name_width, test_suites[i].cases[j].name, flag_buf,
                       perm_buf);
            }
        }
    }
...
static void list_suite_paths(void)
{
    ...
    for (size_t t = 0; t < test_id_count; t++)
    {
        for (size_t i = 0; i < TEST_SUITE_COUNT; i++)
        {
            ...
            printf("%-*s  %s\n", name_width, test_suites[i].name, test_suites[i].path);
        }
    }
}
    ...
    for (size_t t = 0; t < test_id_count; t++)
    {
        for (size_t i = 0; i < TEST_SUITE_COUNT; i++)
        {
            for (size_t j = 0; j < test_suites[i].case_count; j++)
            {
                ...
                printf("%-*s  %s\n", name_width, test_suites[i].cases[j].name,
...
static void list_defines_add(struct list_defines_defines* defines, size_t d)
{
    const char* name = test_define_name(d);
    ...
    for (size_t i = 0; i < defines->define_count; i++)
    {
        if (strcmp(defines->defines[i].name, name) == 0)
        {
            ...
            *(intmax_t*)mappend((void**)&defines->defines[i].values, sizeof(intmax_t),
    ...
    // new define?
    struct list_defines_define* define =
        mappend((void**)&defines->defines, sizeof(struct list_defines_define),
                &defines->define_count, &defines->define_capacity);
    define->name = name;
...
void perm_list_defines(void* data, const struct test_suite* suite, const struct test_case* case_,
                       const test_powerloss_t* powerloss)
{
    ...
    for (size_t d = 0; d < lfs_max(suite->define_count, TEST_IMPLICIT_DEFINE_COUNT); d++)
    {
        if (d < TEST_IMPLICIT_DEFINE_COUNT || test_define_ispermutation(d))
        {
...
void perm_list_permutation_defines(void* data, const struct test_suite* suite,
                                   const struct test_case* case_, const test_powerloss_t* powerloss)
{
    struct list_defines_defines* defines = data;
    (void)suite;
    (void)case_;
    ...
    for (size_t t = 0; t < test_id_count; t++)
    {
        for (size_t i = 0; i < TEST_SUITE_COUNT; i++)
        {
            ...
            for (size_t j = 0; j < test_suites[i].case_count; j++)
            {
                ...
                case_forperm(&test_suites[i], &test_suites[i].cases[j], test_ids[t].defines,
                             test_ids[t].define_count, test_ids[t].cycles, test_ids[t].cycle_count,
                             perm_list_permutation_defines, &defines);
            }
        }
    }
...
static void list_geometries(void)
{
    ...
    printf("%-*s  %7s %7s %7s %7s %11s\n", name_width, "geometry", "read", "prog", "erase", "count",
           "size");
    for (size_t g = 0; builtin_geometries[g].name; g++)
    {
        test_define_geometry(&builtin_geometries[g]);
        test_define_flush();
        printf("%-*s  %7ju %7ju %7ju %7ju %11ju\n", name_width, builtin_geometries[g].name,
               READ_SIZE, PROG_SIZE, ERASE_SIZE, ERASE_COUNT, ERASE_SIZE * ERASE_COUNT);
    }
}
...
static void run_powerloss_linear(const lfs_emubd_powercycles_t* cycles, size_t cycle_count,
                                 const struct test_suite* suite, const struct test_case* case_)
{
    (void)cycles;
    (void)cycle_count;
...
static void run_powerloss_log(const lfs_emubd_powercycles_t* cycles, size_t cycle_count,
                              const struct test_suite* suite, const struct test_case* case_)
{
    (void)cycles;
    (void)cycle_count;
                                 ...
                                 const struct test_suite* suite, const struct test_case* case_)
{
    ...
    while (true)
    {
        ...
        i += 1;
        lfs_emubd_setpowercycles(&cfg, (i < cycle_count) ? cycles[i] : 0);
    }
...
static void powerloss_exhaustive_branch(void* c)
{
    ...
    // append to branches
    lfs_emubd_t* branch = mappend((void**)&state->branches, sizeof(lfs_emubd_t),
                                  &state->branch_count, &state->branch_capacity);
    if (!branch)
    {
...
static void run_powerloss_exhaustive_layer(struct powerloss_exhaustive_cycles* cycles,
                                           const struct test_suite* suite,
                                           const struct test_case* case_, struct lfs_config* cfg,
                                           struct lfs_emubd_config* bdcfg, size_t depth)
{
    (void)suite;
    ...
    for (size_t i = 0; i < state.branch_count; i++)
    {
        ...
        lfs_emubd_powercycles_t* cycle =
            mappend((void**)&cycles->cycles, sizeof(lfs_emubd_powercycles_t), &cycles->cycle_count,
        ...
        *cycle = i + 1;
        ...
        run_powerloss_exhaustive_layer(cycles, suite, case_, cfg, bdcfg, depth - 1);
...
static void run_powerloss_exhaustive(const lfs_emubd_powercycles_t* cycles, size_t cycle_count,
                                     const struct test_suite* suite, const struct test_case* case_)
{
    (void)cycles;
    (void)suite;
    ...
    run_powerloss_exhaustive_layer(&(struct powerloss_exhaustive_cycles){NULL, 0, 0}, suite, case_,
...
static void list_powerlosses(void)
{
    ...
    {
        printf("%-*s %s\n", name_width, builtin_powerlosses[i].name, builtin_powerlosses_help[i]);
    }
...
void perm_run(void* data, const struct test_suite* suite, const struct test_case* case_,
              const test_powerloss_t* powerloss)
{
    (void)data;

    // skip this step?
    if (!(test_step >= test_step_start && test_step < test_step_stop &&
    ...
    powerloss->run(powerloss->cycles, powerloss->cycle_count, suite, case_);
}
...
const struct option long_opts[] = {
    ...
    {"list-permutation-defines", no_argument, NULL, OPT_LIST_PERMUTATION_DEFINES},
    {"list-implicit-defines", no_argument, NULL, OPT_LIST_IMPLICIT_DEFINES},
...
int main(int argc, char** argv)
{
    ...
    while (true)
    {
        ...
        switch (c)
        {
            ...
            case OPT_HELP:
            {
                ...
                while (long_opts[i].name)
                {
                    ...
                    if (long_opts[i].has_arg == no_argument)
                    {
                        ...
                        {
                            indent = printf("  -%c, --%s ", long_opts[i].val, long_opts[i].name);
                        }
                        else
                        {
                            indent = printf("  --%s ", long_opts[i].name);
                        }
                    }
                    else
                    {
                        ...
                        {
                            indent =
                                printf("  -%c %s, --%s %s ", long_opts[i].val, long_opts[i].name,
                                       long_opts[i].name, long_opts[i].name);
                        }
                        else
                        {
                            indent = printf("  --%s %s ", long_opts[i].name, long_opts[i].name);
                        }
                    }
            ...
            {
                // allocate space
                test_override_t* override =
                    mappend((void**)&test_overrides, sizeof(test_override_t), &test_override_count,
                    ...
                    while (true)
                    {
                        ...
                        if (strncmp(optarg, "range", strlen("range")) == 0)
                        {
                            ...
                            for (intmax_t i = start;
                                 (step < 0) ? i > stop : (uintmax_t)i < (uintmax_t)stop; i += step)
                            {
                                *(intmax_t*)mappend((void**)&override->defines, sizeof(intmax_t),
                                                    &override->permutations, &override_capacity) =
                                    i;
                            }
                        }
                        else if (*optarg != '\0')
            ...
            case OPT_GEOMETRY:
            {
                ...
                {
                    // allocate space
                    test_geometry_t* geometry =
                        mappend((void**)&test_geometries, sizeof(test_geometry_t),
                                &test_geometry_count, &test_geometry_capacity);
                    ...
                    for (size_t i = 0; builtin_geometries[i].name; i++)
                    {
                        if (len == strlen(builtin_geometries[i].name) &&
                            memcmp(optarg, builtin_geometries[i].name, len) == 0)
                        {
                    ...
                    if (*optarg == '{')
                    {
                        ...
                        if (count >= 3)
                        {
                            geometry->defines[READ_SIZE_i] = TEST_LIT(sizes[0]);
                            geometry->defines[PROG_SIZE_i] = TEST_LIT(sizes[1]);
                            geometry->defines[ERASE_SIZE_i] = TEST_LIT(sizes[2]);
                        }
                        else if (count >= 2)
                        {
                            geometry->defines[PROG_SIZE_i] = TEST_LIT(sizes[0]);
                            geometry->defines[ERASE_SIZE_i] = TEST_LIT(sizes[1]);
                        }
                        else
                        {
                            geometry->defines[ERASE_SIZE_i] = TEST_LIT(sizes[0]);
                        }
                        if (count >= 4)
                        {
                            geometry->defines[ERASE_COUNT_i] = TEST_LIT(sizes[3]);
                        }
                        optarg = s;
                        goto geometry_next;
                    }
            ...
            case OPT_POWERLOSS:
            {
                ...
                while (*optarg)
                {
                    ...
                    test_powerloss_t* powerloss =
                        mappend((void**)&test_powerlosses, sizeof(test_powerloss_t),
                                &test_powerloss_count, &test_powerloss_capacity);
                    ...
                    for (size_t i = 0; builtin_powerlosses[i].name; i++)
                    {
                        if (len == strlen(builtin_powerlosses[i].name) &&
                            memcmp(optarg, builtin_powerlosses[i].name, len) == 0)
                        {
                    ...
                    if (*optarg == '{')
                    {
                        ...
                        while (true)
                        {
                            ...
                            *(lfs_emubd_powercycles_t*)mappend(
                                (void**)&cycles, sizeof(lfs_emubd_powercycles_t), &cycle_count,
                                &cycle_capacity) = strtoumax(s, &parsed, 0);
                    ...
                    if (*optarg == ':')
                    {
                        ...
                        while (true)
                        {
                            ...
                            *(lfs_emubd_powercycles_t*)mappend((void**)&cycles,
                                                               sizeof(lfs_emubd_powercycles_t),
                                                               &cycle_count, &cycle_capacity) = x;
                            s = parsed;
                        }
...
getopt_done:;
    ...
    for (; argc > optind; optind++)
    {
        ...
        if (defines_)
        {
            ...
            while (true)
            {
                ...
                if (d >= define_count)
                {
                    ...
                    defines = realloc(defines, ncount * sizeof(test_define_t));
            ...
            if (cycles_)
            {
                ...
                while (*cycles_ != '\0')
                {
                    ...
                    *(lfs_emubd_powercycles_t*)mappend(
                        (void**)&cycles, sizeof(lfs_emubd_powercycles_t), &cycle_count,
                        &cycle_capacity) = leb16_parse(cycles_, &parsed);
                    if (parsed == cycles_)
                    {
                        fprintf(stderr,
                                "error: "
        ...
        *(test_id_t*)mappend((void**)&test_ids, sizeof(test_id_t), &test_id_count,
