...
static lfs_emubd_block_t* lfs_emubd_mutblock(const struct lfs_config* cfg,
                                             lfs_emubd_block_t** block)
{
    ...
    else if (block_)
    {
        ...
        lfs_emubd_block_t* nblock = malloc(sizeof(lfs_emubd_block_t) + bd->cfg->erase_size);
        if (!nblock)
        {
            return NULL;
        }

        memcpy(nblock, block_, sizeof(lfs_emubd_block_t) + bd->cfg->erase_size);
        nblock->rc = 1;
    ...
    else
    {
        ...
        memset(nblock->data, (bd->cfg->erase_value != -1) ? bd->cfg->erase_value : 0,
               bd->cfg->erase_size);
...
int lfs_emubd_create(const struct lfs_config* cfg, const struct lfs_emubd_config* bdcfg)
{
    LFS_EMUBD_TRACE(
        "lfs_emubd_create(%p {.context=%p, "
        ...
        (void*)(uintptr_t)cfg->erase, (void*)(uintptr_t)cfg->sync, (void*)bdcfg, bdcfg->read_size,
        bdcfg->prog_size, bdcfg->erase_size, bdcfg->erase_count, bdcfg->erase_value,
        bdcfg->erase_cycles, bdcfg->badblock_behavior, bdcfg->power_cycles,
        bdcfg->powerloss_behavior, (void*)(uintptr_t)bdcfg->powerloss_cb, bdcfg->powerloss_data,
        bdcfg->track_branches);
    lfs_emubd_t* bd = cfg->context;
    bd->cfg = bdcfg;
    ...
    if (bd->cfg->disk_path)
    {
...
#ifdef _WIN32
        bd->disk->fd = open(bd->cfg->disk_path, O_RDWR | O_CREAT | O_BINARY, 0666);
#else
        bd->disk->fd = open(bd->cfg->disk_path, O_RDWR | O_CREAT, 0666);
#endif
        if (bd->disk->fd < 0)
        {
            int err = -errno;
        ...
        if (bd->cfg->erase_value != -1)
        {
            ...
            memset(bd->disk->scratch, bd->cfg->erase_value, bd->cfg->erase_size);
            ...
            {
                ssize_t res = write(bd->disk->fd, bd->disk->scratch, bd->cfg->erase_size);
                if (res < 0)
                {
                    int err = -errno;
                    LFS_EMUBD_TRACE("lfs_emubd_create -> %d", err);
...
static int lfs_emubd_powerloss(const struct lfs_config* cfg)
{
    ...
    if (bd->cfg->powerloss_behavior == LFS_EMUBD_POWERLOSS_OOO && bd->ooo_block != -1)
    {
        ...
        {
            off_t res1 = lseek(bd->disk->fd, (off_t)bd->ooo_block * bd->cfg->erase_size, SEEK_SET);
            if (res1 < 0)
            {
                return -errno;
            }
            ...
            ssize_t res2 = write(
                bd->disk->fd,
                (bd->blocks[bd->ooo_block]) ? bd->blocks[bd->ooo_block]->data : bd->disk->scratch,
                bd->cfg->erase_size);
            if (res2 < 0)
            {
                return -errno;
                   ...
                   lfs_size_t size)
{
    ...
    {
        // zero for consistency
        memset(buffer, (bd->cfg->erase_value != -1) ? bd->cfg->erase_value : 0, size);
    }
    ...
    {
        int err = nanosleep(&(struct timespec){.tv_sec = bd->cfg->read_sleep / 1000000000,
                   ...
                   const void* buffer, lfs_size_t size)
{
    ...
    if (bd->cfg->erase_cycles && b->wear >= bd->cfg->erase_cycles)
    {
        if (bd->cfg->badblock_behavior == LFS_EMUBD_BADBLOCK_PROGERROR)
        {
            LFS_EMUBD_TRACE("lfs_emubd_prog -> %d", LFS_ERR_CORRUPT);
            return LFS_ERR_CORRUPT;
        }
        else if (bd->cfg->badblock_behavior == LFS_EMUBD_BADBLOCK_PROGNOOP ||
                 bd->cfg->badblock_behavior == LFS_EMUBD_BADBLOCK_ERASENOOP)
        {
    ...
    if (bd->disk)
    {
        off_t res1 = lseek(bd->disk->fd, (off_t)block * bd->cfg->erase_size + (off_t)off, SEEK_SET);
        if (res1 < 0)
        {
            int err = -errno;
    ...
    {
        int err = nanosleep(&(struct timespec){.tv_sec = bd->cfg->prog_sleep / 1000000000,
...
{
    LFS_EMUBD_TRACE("lfs_emubd_erase(%p, 0x%" PRIx32 " (%" PRIu32 "))", (void*)cfg, block,
                    ((lfs_emubd_t*)cfg->context)->cfg->erase_size);
    lfs_emubd_t* bd = cfg->context;
    ...
    if (bd->cfg->erase_cycles)
    {
        ...
        {
            if (bd->cfg->badblock_behavior == LFS_EMUBD_BADBLOCK_ERASEERROR)
            {
                LFS_EMUBD_TRACE("lfs_emubd_erase -> %d", LFS_ERR_CORRUPT);
                return LFS_ERR_CORRUPT;
            }
            else if (bd->cfg->badblock_behavior == LFS_EMUBD_BADBLOCK_ERASENOOP)
            {
    ...
    if (bd->cfg->erase_value != -1)
    {
        ...
        if (bd->disk)
        {
            off_t res1 = lseek(bd->disk->fd, (off_t)block * bd->cfg->erase_size, SEEK_SET);
            if (res1 < 0)
            {
                int err = -errno;
            ...
            ssize_t res2 = write(bd->disk->fd, bd->disk->scratch, bd->cfg->erase_size);
            if (res2 < 0)
            {
                int err = -errno;
                LFS_EMUBD_TRACE("lfs_emubd_erase -> %d", err);
    ...
    {
        int err = nanosleep(&(struct timespec){.tv_sec = bd->cfg->erase_sleep / 1000000000,
...
static int lfs_emubd_crc_(const struct lfs_config* cfg, lfs_block_t block, uint32_t* crc)
{
    lfs_emubd_t* bd = cfg->context;
    ...
    {
        uint8_t erase_value = (bd->cfg->erase_value != -1) ? bd->cfg->erase_value : 0;
...
int lfs_emubd_crc(const struct lfs_config* cfg, lfs_block_t block, uint32_t* crc)
{
    LFS_EMUBD_TRACE("lfs_emubd_crc(%p, %" PRIu32 ", %p)", (void*)cfg, block, crc);
...
lfs_emubd_swear_t lfs_emubd_wear(const struct lfs_config* cfg, lfs_block_t block)
{
...
int lfs_emubd_setwear(const struct lfs_config* cfg, lfs_block_t block, lfs_emubd_wear_t wear)
{
    LFS_EMUBD_TRACE("lfs_emubd_setwear(%p, %" PRIu32 ", %" PRIi32 ")", (void*)cfg, block, wear);
    lfs_emubd_t* bd = cfg->context;
...
lfs_emubd_spowercycles_t lfs_emubd_powercycles(const struct lfs_config* cfg)
{
...
int lfs_emubd_setpowercycles(const struct lfs_config* cfg, lfs_emubd_powercycles_t power_cycles)
{
    LFS_EMUBD_TRACE("lfs_emubd_setpowercycles(%p, %" PRIi32 ")", (void*)cfg, power_cycles);
    lfs_emubd_t* bd = cfg->context;
