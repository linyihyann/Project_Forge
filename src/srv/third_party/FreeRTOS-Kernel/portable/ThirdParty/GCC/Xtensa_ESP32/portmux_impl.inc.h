...
PORTMUX_AQUIRE_MUX_FN_NAME(portMUX_TYPE* mux, int timeout_cycles)
{
    ...
    {
        ets_printf(
            "ERROR: vPortCPUAcquireMutex: mux %p is uninitialized (0x%X)! Called from %s line "
            "%d.\n",
 ...
 */
    ...
    assert(
        res == coreID ||
        res ==
            portMUX_FREE_VAL); /* any other value implies memory corruption or uninitialized mux */
    ...
    else
    {
        ets_printf("Recursive lock: count=%d last non-recursive lock %s line %d, curr %s line %d\n",
                   mux->count - 1, mux->lastLockedFn, mux->lastLockedLine, fnName, line);
    }
