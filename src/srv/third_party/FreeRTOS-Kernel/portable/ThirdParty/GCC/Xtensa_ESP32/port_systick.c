...
_Static_assert(SOC_CPU_CORES_NUM <= SOC_SYSTIMER_ALARM_NUM - 1,
               "the number of cores must match the number of core alarms in SYSTIMER");
...
void vPortSetupTimer(void)
{
    ...
    ESP_ERROR_CHECK(esp_intr_alloc(ETS_SYSTIMER_TARGET0_EDGE_INTR_SOURCE + cpuid,
                                   ESP_INTR_FLAG_IRAM | level, SysTickIsrHandler, &systimer_hal,
                                   NULL));
    ...
    if (cpuid == 0)
    {
        ...
        {
            systimer_hal_counter_can_stall_by_cpu(&systimer_hal, SYSTIMER_LL_COUNTER_OS_TICK, cpuid,
                                                  false);
        }
        ...
        for (cpuid = 0; cpuid < portNUM_PROCESSORS; ++cpuid)
        {
            ...
            /* configure the timer */
            systimer_hal_connect_alarm_counter(&systimer_hal, alarm_id,
            ...
            if (cpuid == 0)
            {
                ...
                systimer_hal_counter_value_advance(&systimer_hal, SYSTIMER_LL_COUNTER_OS_TICK,
                                                   1000000UL / CONFIG_FREERTOS_HZ / 2);
#endif
            }
        }
    }
 ...
 * If a timer interrupt is ever serviced more than one tick late, it is necessary to process
 * multiple ticks.
 */
IRAM_ATTR void SysTickIsrHandler(void* arg)
{
    ...
    do
    {
        ...
        uint32_t diff = systimer_hal_get_counter_value(systimer_hal, SYSTIMER_LL_COUNTER_OS_TICK) /
                            systimer_ll_get_alarm_period(systimer_hal->dev, alarm_id) -
