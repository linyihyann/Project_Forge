                      ...
                      const struct lfs_filebd_config* bdcfg)
{
    LFS_FILEBD_TRACE(
        ...
        (void*)(uintptr_t)cfg->erase, (void*)(uintptr_t)cfg->sync, path, (void*)bdcfg,
                    ...
                    lfs_size_t size)
{
    ...
    // read
    off_t res1 = lseek(bd->fd, (off_t)block * bd->cfg->erase_size + (off_t)off, SEEK_SET);
    if (res1 < 0)
    {
        int err = -errno;
...
{
    LFS_FILEBD_TRACE("lfs_filebd_erase(%p, 0x%" PRIx32 " (%" PRIu32 "))", (void*)cfg, block,
