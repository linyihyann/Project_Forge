...
static int prvCoreInit(void)
{
    ...
    if (xCoreID == 0)
    {
        ...
        triggerable_setup_interrupt_callback(xKernelTimer, NULL,
                                             RTOS_INTERRUPT_CALLBACK(pxKernelTimerISR));
...
/* Populate the TCBContainer depending on whether we're singlecore or SMP */
    ...
    {
        asm volatile("ldaw %0, dp[pxCurrentTCB]\n\t"
...
#else
    {
        asm volatile("ldaw %0, dp[pxCurrentTCBs]\n\t"
                     : "=r"(xcorePvtTCBContainer)
                     : /* no inputs */
                     : /* no clobbers */
        );
                                   ...
                                   void* pvParameters)
{
    ...
    pxTopOfStack[2] = XS1_SR_IEBLE_MASK | XS1_SR_KEDI_MASK; /* SP[2]  := SSR */
