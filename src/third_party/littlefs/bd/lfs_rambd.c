...
int lfs_rambd_create(const struct lfs_config* cfg, const struct lfs_rambd_config* bdcfg)
{
    LFS_RAMBD_TRACE(
        "lfs_rambd_create(%p {.context=%p, "
        ...
        bdcfg->prog_size, bdcfg->erase_size, bdcfg->erase_count, bdcfg->buffer);
...
{
    LFS_RAMBD_TRACE("lfs_rambd_erase(%p, 0x%" PRIx32 " (%" PRIu32 "))", (void*)cfg, block,
                    ((lfs_rambd_t*)cfg->context)->cfg->erase_size);
    lfs_rambd_t* bd = cfg->context;
