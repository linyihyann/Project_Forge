...
#define BENCH_SUITE_COUNT ((size_t)(&__stop__bench_suites - &__start__bench_suites))
...
#define BENCH_DEF(k, v) [k##_i] = {bench_define_##k, NULL},
...
const char* bench_define_name(size_t define)
{
    ...
    {
        if (define < bench_define_names[i].count && bench_define_names[i].names &&
...
bool bench_define_ispermutation(size_t define)
{
    ...
    {
        if (define < bench_define_maps[i].count && bench_define_maps[i].defines[define].cb)
        {
            return true;
        }
    }
...
intmax_t bench_define(size_t define)
{
    ...
    for (size_t i = 0; i < BENCH_DEFINE_MAP_COUNT; i++)
    {
        ...
        {
            intmax_t v =
                bench_define_maps[i].defines[define].cb(bench_define_maps[i].defines[define].data);
...
void bench_define_flush(void)
{
    ...
    memset(bench_define_cache_mask, 0,
           sizeof(unsigned) *
               ((bench_define_cache_count + (8 * sizeof(unsigned)) - 1) / (8 * sizeof(unsigned))));
}
...
{
    bench_define_maps[BENCH_DEFINE_MAP_GEOMETRY] =
        (bench_define_map_t){geometry->defines, BENCH_GEOMETRY_DEFINE_COUNT};
}
...
{
    bench_define_names[BENCH_DEFINE_NAMES_SUITE] =
        (bench_define_names_t){suite->define_names, suite->define_count};

    // make sure our cache is large enough
    if (lfs_max(suite->define_count, BENCH_IMPLICIT_DEFINE_COUNT) > bench_define_cache_count)
    {
        // align to power of two to avoid any superlinear growth
        size_t ncount = 1 << lfs_npw2(lfs_max(suite->define_count, BENCH_IMPLICIT_DEFINE_COUNT));
        bench_define_cache = realloc(bench_define_cache, ncount * sizeof(intmax_t));
        bench_define_cache_mask = realloc(
    ...
    if (bench_override_count > 0)
    {
        ...
        {
            for (size_t d = 0; d < lfs_max(suite->define_count, BENCH_IMPLICIT_DEFINE_COUNT); d++)
            {
                // define name match?
        ...
        if (count * permutations > bench_override_define_capacity)
        {
            ...
            bench_override_defines =
                realloc(bench_override_defines, sizeof(bench_define_t) * ncapacity);
            bench_override_define_capacity = ncapacity;
        }

        // zero unoverridden defines
        memset(bench_override_defines, 0, sizeof(bench_define_t) * count * permutations);
                ...
                if (name && strcmp(name, bench_overrides[i].name) == 0)
                {
                    ...
                    for (size_t j = 0; j < permutations; j++)
                    {
                        bench_override_defines[j * count + d] = BENCH_LIT(
                            bench_overrides[i].defines[(j / p) % bench_overrides[i].permutations]);
                    }
...
void bench_define_perm(const struct bench_suite* suite, const struct bench_case* case_, size_t perm)
{
    if (case_->defines)
    {
        bench_define_maps[BENCH_DEFINE_MAP_PERMUTATION] =
            (bench_define_map_t){case_->defines + perm * suite->define_count, suite->define_count};
    }
    else
    {
        bench_define_maps[BENCH_DEFINE_MAP_PERMUTATION] = (bench_define_map_t){NULL, 0};
    }
}
...
void bench_define_override(size_t perm)
{
    bench_define_maps[BENCH_DEFINE_MAP_OVERRIDE] = (bench_define_map_t){
        bench_override_defines + perm * bench_override_define_count, bench_override_define_count};
}

void bench_define_explicit(const bench_define_t* defines, size_t define_count)
{
    bench_define_maps[BENCH_DEFINE_MAP_EXPLICIT] = (bench_define_map_t){defines, define_count};
}
...
#endif
void* bench_trace_backtrace_buffer[BENCH_TRACE_BACKTRACE_BUFFER_SIZE / sizeof(void*)];
...
void bench_trace(const char* fmt, ...)
{
    if (bench_trace_path)
    {
        ...
        if (!bench_trace_file)
        {
            ...
            {
                fd = open(bench_trace_path, O_WRONLY | O_CREAT | O_APPEND | O_NONBLOCK, 0666);
                if (fd < 0)
                {
                    return;
                }
            ...
            FILE* f = fdopen(fd, "a");
            assert(f);
            int err = setvbuf(f, NULL, _IOFBF, BENCH_TRACE_BACKTRACE_BUFFER_SIZE);
            assert(!err);
            bench_trace_file = f;
        }
        ...
        if (bench_trace_backtrace)
        {
            // print backtrace
            size_t count =
                backtrace(bench_trace_backtrace_buffer, BENCH_TRACE_BACKTRACE_BUFFER_SIZE);
...
static void perm_printid(const struct bench_suite* suite, const struct bench_case* case_)
{
    (void)suite;
    // case[:permutation]
    printf("%s:", case_->name);
...
bool bench_seen_insert(bench_seen_t* seen, const struct bench_suite* suite,
...
static void case_forperm(const struct bench_suite* suite, const struct bench_case* case_,
                         const bench_define_t* defines, size_t define_count,
                         void (*cb)(void* data, const struct bench_suite* suite,
                                    const struct bench_case* case_),
                         void* data)
{
...
void perm_count(void* data, const struct bench_suite* suite, const struct bench_case* case_)
{
...
static void summary(void)
{
    ...
    for (size_t t = 0; t < bench_id_count; t++)
    {
        for (size_t i = 0; i < BENCH_SUITE_COUNT; i++)
        {
            ...
            for (size_t j = 0; j < bench_suites[i].case_count; j++)
            {
                ...
                if (bench_ids[t].name &&
                    !(strcmp(bench_ids[t].name, bench_suites[i].name) == 0 ||
                      strcmp(bench_ids[t].name, bench_suites[i].cases[j].name) == 0))
                {
                    continue;
                }

                cases += 1;
                case_forperm(&bench_suites[i], &bench_suites[i].cases[j], bench_ids[t].defines,
                             bench_ids[t].define_count, perm_count, &perms);
            }
    ...
    char flag_buf[64];
    sprintf(flag_buf, "%s%s", (flags & BENCH_REENTRANT) ? "r" : "", (!flags) ? "-" : "");
...
static void list_suites(void)
{
    ...
    for (size_t t = 0; t < bench_id_count; t++)
    {
        for (size_t i = 0; i < BENCH_SUITE_COUNT; i++)
        {
            ...
            sprintf(perm_buf, "%zu/%zu", perms.filtered, perms.total);
            char flag_buf[64];
            sprintf(flag_buf, "%s%s", (bench_suites[i].flags & BENCH_REENTRANT) ? "r" : "",
                    (!bench_suites[i].flags) ? "-" : "");
            printf("%-*s  %7s %7zu %11s\n", name_width, bench_suites[i].name, flag_buf, cases,
                   perm_buf);
        }
    }
}
        ...
        for (size_t i = 0; i < BENCH_SUITE_COUNT; i++)
        {
            ...
            for (size_t j = 0; j < bench_suites[i].case_count; j++)
            {
                ...
                sprintf(flag_buf, "%s%s",
                        (bench_suites[i].cases[j].flags & BENCH_REENTRANT) ? "r" : "",
                        (!bench_suites[i].cases[j].flags) ? "-" : "");
                printf("%-*s  %7s %11s\n", name_width, bench_suites[i].cases[j].name, flag_buf,
                       perm_buf);
            }
        }
    }
...
static void list_suite_paths(void)
{
    ...
    for (size_t t = 0; t < bench_id_count; t++)
    {
        for (size_t i = 0; i < BENCH_SUITE_COUNT; i++)
        {
            ...
            printf("%-*s  %s\n", name_width, bench_suites[i].name, bench_suites[i].path);
        }
    }
}
    ...
    for (size_t t = 0; t < bench_id_count; t++)
    {
        for (size_t i = 0; i < BENCH_SUITE_COUNT; i++)
        {
            for (size_t j = 0; j < bench_suites[i].case_count; j++)
            {
                ...
                printf("%-*s  %s\n", name_width, bench_suites[i].cases[j].name,
...
void perm_list_defines(void* data, const struct bench_suite* suite, const struct bench_case* case_)
{
    ...
    for (size_t d = 0; d < lfs_max(suite->define_count, BENCH_IMPLICIT_DEFINE_COUNT); d++)
    {
        if (d < BENCH_IMPLICIT_DEFINE_COUNT || bench_define_ispermutation(d))
        {
...
void perm_list_permutation_defines(void* data, const struct bench_suite* suite,
                                   const struct bench_case* case_)
{
    struct list_defines_defines* defines = data;
    (void)suite;
    ...
    for (size_t t = 0; t < bench_id_count; t++)
    {
        for (size_t i = 0; i < BENCH_SUITE_COUNT; i++)
        {
            ...
            for (size_t j = 0; j < bench_suites[i].case_count; j++)
            {
                ...
                case_forperm(&bench_suites[i], &bench_suites[i].cases[j], bench_ids[t].defines,
                             bench_ids[t].define_count, perm_list_defines, &defines);
            }
        }
    }
    ...
    for (size_t t = 0; t < bench_id_count; t++)
    {
        for (size_t i = 0; i < BENCH_SUITE_COUNT; i++)
        {
            ...
            for (size_t j = 0; j < bench_suites[i].case_count; j++)
            {
                ...
                case_forperm(&bench_suites[i], &bench_suites[i].cases[j], bench_ids[t].defines,
                             bench_ids[t].define_count, perm_list_permutation_defines, &defines);
            }
        }
    }
...
void perm_run(void* data, const struct bench_suite* suite, const struct bench_case* case_)
{
    (void)data;

    // skip this step?
    if (!(bench_step >= bench_step_start && bench_step < bench_step_stop &&
    ...
    printf("finished ");
    perm_printid(suite, case_);
    printf(" %" PRIu64 " %" PRIu64 " %" PRIu64, bench_readed, bench_proged, bench_erased);
    printf("\n");
    ...
    for (size_t t = 0; t < bench_id_count; t++)
    {
        for (size_t i = 0; i < BENCH_SUITE_COUNT; i++)
        {
            ...
            for (size_t j = 0; j < bench_suites[i].case_count; j++)
            {
                ...
                case_forperm(&bench_suites[i], &bench_suites[i].cases[j], bench_ids[t].defines,
                             bench_ids[t].define_count, perm_run, NULL);
            }
        }
    }
}
...
int main(int argc, char** argv)
{
    ...
    while (true)
    {
        ...
        switch (c)
        {
            ...
            {
                // allocate space
                bench_override_t* override =
                    mappend((void**)&bench_overrides, sizeof(bench_override_t),
                            &bench_override_count, &bench_override_capacity);
            ...
            case OPT_GEOMETRY:
            {
                ...
                {
                    // allocate space
                    bench_geometry_t* geometry =
                        mappend((void**)&bench_geometries, sizeof(bench_geometry_t),
                                &bench_geometry_count, &bench_geometry_capacity);
                    ...
                    if (*optarg == '{')
                    {
                        ...
                        if (count >= 3)
                        {
                            geometry->defines[READ_SIZE_i] = BENCH_LIT(sizes[0]);
                            geometry->defines[PROG_SIZE_i] = BENCH_LIT(sizes[1]);
                            geometry->defines[ERASE_SIZE_i] = BENCH_LIT(sizes[2]);
                        }
                        else if (count >= 2)
                        {
                            geometry->defines[PROG_SIZE_i] = BENCH_LIT(sizes[0]);
                            geometry->defines[ERASE_SIZE_i] = BENCH_LIT(sizes[1]);
                        }
                        else
                        {
                            geometry->defines[ERASE_SIZE_i] = BENCH_LIT(sizes[0]);
                        }
                        if (count >= 4)
                        {
                            geometry->defines[ERASE_COUNT_i] = BENCH_LIT(sizes[3]);
                        }
                        optarg = s;
                        goto geometry_next;
                    }
...
getopt_done:;
    ...
    for (; argc > optind; optind++)
    {
        ...
        if (defines_)
        {
            ...
            while (true)
            {
                ...
                if (d >= define_count)
                {
                    ...
                    defines = realloc(defines, ncount * sizeof(bench_define_t));
        ...
        *(bench_id_t*)mappend((void**)&bench_ids, sizeof(bench_id_t), &bench_id_count,
